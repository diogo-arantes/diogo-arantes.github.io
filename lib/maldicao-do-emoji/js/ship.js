...
function checkBossCollision(x, y, size) {
    if (currentBoss.name === "circular") {
        ...
        // Distance-based collision for "still" boss
        let dx = x - bossX;
        let dy = y - bossY;
        let distance = Math.sqrt(dx * dx + dy * dy);

        return distance < (size + bossSize) / 2;
    }
